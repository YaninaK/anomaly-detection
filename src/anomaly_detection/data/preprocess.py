...
class Preprocess:
    ...
    def adjust_address_data(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        5. Для соотнесения с базой данных объектов, адрес объекта типа Другое строение г Уфа, ул. Уфимское шоссе, д.20
           изменен в признаке Адрес объекта 2: г Уфа, ул. Уфимское шоссе, д.20 корп.1
        ...
        data.loc[
            (data["Адрес объекта"] == "г Уфа, ул. Уфимское шоссе, д.20")
            & (data["Тип объекта"] == "Другое строение"),
            "Адрес объекта 2",
        ] = "г Уфа, ул. Уфимское шоссе, д.20 корп.1"
    ...
    def adjust_address_buildings(self, buildings: pd.DataFrame) -> pd.DataFrame:
        ...
        ## 3.2 Учебное заведение, комбинат, центр
    ...
    def adjust_subobjects_data(
        ...
        """
        Укрупняет адрес объекта в базе учета теплопотребления data (признак Адрес объекта 2),
        убирая подобъект/ подъезд, чтобы привести в соответствие с базой объектов buildings
        (признак Адрес объекта 2).
        """
        ...
        data.loc[
            data["Адрес объекта"] == "г Уфа, ул. Добролетная, д.7 корп.2, Подъезд №1",
            "Адрес объекта 2",
        ] = "г Уфа, ул. Добролетная, д.7 корп.2"
        data.loc[
            data["Адрес объекта"]
            == "г Уфа, ул. Юрия Гагарина, д.41 корп.3, Подъезд №1",
            "Адрес объекта 2",
        ] = "г Уфа, ул. Юрия Гагарина, д.41 корп.3"
        data.loc[
            (
                data["Адрес объекта"]
                == "г Уфа, ул. Ахметова, д.322, Гаражи (ЧЖД) №987001"
            )
            & (data["Тип объекта"] == "Гаражи"),
            "Адрес объекта 2",
